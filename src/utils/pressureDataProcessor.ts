...
// Define sensor regions based on the Pedar insole layout image
const REGIONS = {
  heel: Array.from({ length: 26 }, (_, i) => i + 1), // 1-26 for heel (orange region)
  midfoot: Array.from({ length: 28 }, (_, i) => i + 27), // 27-54 for midfoot (light blue region)
  forefoot: Array.from({ length: 28 }, (_, i) => i + 55), // 55-82 for forefoot (green region)
  toes: Array.from({ length: 17 }, (_, i) => i + 83) // 83-99 for toes (purple region)
};
