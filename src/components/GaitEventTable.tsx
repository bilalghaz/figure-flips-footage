...
const GaitEventTable: React.FC<GaitEventTableProps> = ({ data, thresholds }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle>Step Time Details</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>#</TableHead>
                <TableHead>Left Step Time (s)</TableHead>
                <TableHead>Right Step Time (s)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {Array.from({ length: Math.max(gaitParameters.stepTime.left.values.length, gaitParameters.stepTime.right.values.length) }).map((_, index) => (
                <TableRow key={`step-${index}`}>
                  <TableCell>{index + 1}</TableCell>
                  <TableCell>
                    {index < gaitParameters.stepTime.left.values.length 
                      ? gaitParameters.stepTime.left.values[index].toFixed(2) 
                      : '-'}
                  </TableCell>
                  <TableCell>
                    {index < gaitParameters.stepTime.right.values.length 
                      ? gaitParameters.stepTime.right.values[index].toFixed(2) 
                      : '-'}
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle>Stride Time Details</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>#</TableHead>
                <TableHead>Left Stride Time (s)</TableHead>
                <TableHead>Right Stride Time (s)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {Array.from({ length: Math.max(gaitParameters.strideTime.left.values.length, gaitParameters.strideTime.right.values.length) }).map((_, index) => (
                <TableRow key={`stride-${index}`}>
                  <TableCell>{index + 1}</TableCell>
                  <TableCell>
                    {index < gaitParameters.strideTime.left.values.length 
                      ? gaitParameters.strideTime.left.values[index].toFixed(2) 
                      : '-'}
                  </TableCell>
                  <TableCell>
                    {index < gaitParameters.strideTime.right.values.length 
                      ? gaitParameters.strideTime.right.values[index].toFixed(2) 
                      : '-'}
                  </TableCell>
                </TableRow>
              ))}
